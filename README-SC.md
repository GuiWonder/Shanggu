...
### 3. 🔁 自动简转繁字体（ST 版本）
字体具备“简入繁出”功能。在处理“一简对多繁”时，字体借助 OpenType 特性，实现了基于上下文语境的“一简对多繁”动态匹配。以“干”字为例：当检测到后接“活”字时，系统会自动触发“干→幹”的定向替换，输出“幹活”；而当后接“燥”字时，则触发“干→乾”的替换，输出“乾燥”。这种基于语境的动态字形调用，有效提升了文本转换的精准度。<br />
...
## 📦 字体的制作过程
在[思源黑体 (Source Han Sans)](https://github.com/adobe-fonts/source-han-sans)、[思源宋体 (Source Han Serif)](https://github.com/adobe-fonts/source-han-serif) 日文版的基础上，利用 locl 本地化特性与 UVS（变体选择器）数据表，挑选调取字库底层隐藏的、符合传承规范的异体字形，重新编写至底层 cmap（字符映射表）中，作为默认显示字形。<br />
在映射关系确立后，进一步增补了许多重绘字形。其中，黑体增补 6,537 个字形（包含 817 个直接引入[秋空󠄁黑体 (ChiuKong Gothic)](https://github.com/ChiuMing-Neko/ChiuKongGothic)字形）；明体则同步完成了 6,659 个字形的重绘。
